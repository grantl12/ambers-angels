...
| :--- | :--- |
| **No Video Archiving** | Phone scanning runs entirely on-device — non-matching frames never leave the volunteer's phone; one frame uploads only as evidence on a watchlist hit. Drone feeds are processed server-side; non-matching frames are deleted immediately after processing. High-confidence match frames ("golden frames") are saved as time-limited evidence for up to one year, then auto-purged. |
...
| Raw Video Frames (phones) | On-device inference; one frame retained only on a confirmed watchlist hit | **Not retained** for non-matches — never leaves the device; match evidence retained up to 1 year, same as drone golden frames | Device only for non-matches; processing system + coordinators for hit evidence |
