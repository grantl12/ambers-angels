...
**`deploy.yml` never installs backend Python deps — same wrong-owner trap can hit `pip` too**
Unlike the web frontend (`npm ci` on every deploy), `deploy.yml` never runs `pip install` for `backend/requirements.txt`. Discovered 2026-07-28: `feedparser`, `beautifulsoup4`, and `spacy` were added to `requirements.txt` for the BOLO crawler but never actually installed on the server — every fetch silently hit the code's own `except ImportError` fallback for the entire life of that feature, while `last_crawled_at` kept updating on schedule and made it look healthy. Fixed by adding `pip3 install --user -q -r backend/requirements.txt` to `deploy.yml`, run before `run_migration.py`. When manually installing on the server, also check `/home/ambers-angels/.local` ownership first — found it entirely owned by `www-data` (not `ambers-angels`) from some earlier process, which made `pip install --user` fail with `Permission denied` on write. Same root cause as the npm root-owned-files trap above, different culprit user: `chown -R ambers-angels:ambers-angels /home/ambers-angels/.local /home/ambers-angels/.cache` before retrying.
