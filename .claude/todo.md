...
### Capture Interval — Known Issues
`captureIntervalSec` defaults to 3s (`mobile/src/lib/settings.ts:23`). ScanService.kt default is 1500ms but overwritten by intent extra on start. **Broken**: `/ingest/frame` returns `capture_interval_ms` (800ms/1500ms) but `CameraScreen.tsx` never reads it — timers are fixed `setInterval` calls. Fix: consume `capture_interval_ms` from server response to drive active camera interval; move `captureIntervalSec` setting to Android background service only (battery tradeoff). Also: gate Plate Recognizer on ALPR ≥ 70% **and** plate is on active watchlist — cuts API calls ~95%.
