...
def _area_to_state_centroid(area: str | None) -> tuple[float, float] | None:
    """Fallback: extract the most-common state code from an area string and return its centroid."""
    if not area:
        return None
    codes = _STATE_RE.findall(area)
    if not codes:
        return None
    from collections import Counter
    dominant = Counter(codes).most_common(1)[0][0]
    return _STATE_CENTROIDS.get(dominant)
...
async def _add_vehicle_target(session_factory, alert: dict) -> bool:
    ...
    centroid = (
        _polygon_to_centroid(alert.get("polygon"))
        or _area_to_state_centroid(alert.get("area"))
    )
            ...
            result = await session.execute(
                text("""
                    ...
                    ON CONFLICT (fema_identifier) DO UPDATE
                        SET centroid_lat = COALESCE(vehicle_targets.centroid_lat, EXCLUDED.centroid_lat),
                            centroid_lng = COALESCE(vehicle_targets.centroid_lng, EXCLUDED.centroid_lng)
                """),
                {
