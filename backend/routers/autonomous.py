...
from services.autonomous_mission_service import (
    ...
    create_water_search_mission,
    get_mission,
    list_missions,
    request_relook,
    update_mission_status,
)
from services.water_body_service import fetch_nearby_water_bodies
from services.waypoint_generator import check_vlos_radius, generate_lawnmower, generate_observation_point, generate_water_perimeter
...
class PlanWaterSearchRequest(BaseModel):
    alert_id: str
    drone_id: int
    water_body_id: str
...
@router.post("/autonomous/plan-water-search")
async def plan_water_search_endpoint(
    req: PlanWaterSearchRequest,
    payload: dict = Depends(get_current_pilot),
    db: AsyncSession = Depends(get_async_db),
):
    """
    Plan a water body perimeter search mission for Purple Alert scenarios.

    The drone traces the shoreline at buffer_m offset (default 20m for tree
    clearance), then hovers at the last waypoint (ready_for_control). No
    ALPR/YOLO — coordinator visually searches via live camera feed.
    """
    if req.operation_mode not in VALID_OPERATION_MODES:
        raise HTTPException(status_code=422, detail=f"Invalid operation_mode '{req.operation_mode}'.")
    ...
    if req.operation_mode == "vlos":
        ...
        test_waypoints = generate_water_perimeter(
            req.polygon_geojson,
        ...
        if not test_waypoints:
            raise HTTPException(status_code=422, detail="Could not generate perimeter — water body polygon too small or malformed.")
        ...
        if not within:
            raise HTTPException(
                status_code=422,
                detail=f"Water perimeter extends {dist:.0f} m from home, exceeding {drone['vlos_radius_m']} m VLOS limit.",
            )

    mission = await create_water_search_mission(
        db,
        alert_id=req.alert_id,
        drone_id=req.drone_id,
        water_body_id=req.water_body_id,
        water_body_name=req.water_body_name,
        polygon_geojson=req.polygon_geojson,
        buffer_m=req.buffer_m,
        altitude_m=req.altitude_m,
        speed_mps=req.speed_mps,
    ...
    asyncio.create_task(write_audit_async(
        database.AsyncSessionLocal,
        username=payload.get("sub", "unknown"),
        action="water_search_mission_dispatched",
        details={
...
@router.get("/autonomous/water-bodies")
async def get_nearby_water_bodies(
    lat: float,
    lng: float,
    radius_km: float = 5.0,
    payload: dict = Depends(get_current_pilot),
    ...
    """
    Return water bodies near (lat, lng) ranked by distance.
    Used by coordinators during Purple Alert to identify search targets.
    Backed by cached Overpass API data (24h TTL).
    """
    if radius_km < 0.1 or radius_km > 50:
        raise HTTPException(status_code=422, detail="radius_km must be 0.1–50.")
    return await fetch_nearby_water_bodies(db, lat, lng, radius_km=radius_km)
